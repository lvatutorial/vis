...
Graph3d.prototype._redrawInfo = function() {
  ...
  ctx.font = '24px arial'; // TODO: put in options
  ctx.lineStyle = 'gray';
  ctx.fillStyle = 'gray';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
...
Graph3d.prototype._redrawAxis = function() {
  ...
  ctx.font = 32 / this.camera.getArmLength() + 'px arial';

  // calculate the length for the short grid lines
  var gridLenX   = 0.025 / this.scale.x;
  var gridLenY   = 0.025 / this.scale.y;
  var textMargin = 10 / this.camera.getArmLength(); // px
  ...
  while (!step.end()) {
    ...
    if (this.showXAxis) {
      ...
      point3d = new Point3d(x+0.5, yText, zRange.min);
      if (this.xValueLabel(x)){
  ...
  while (!step.end()) {
    ...
    if (this.showYAxis) {
      ...
      point3d = new Point3d(xText, y+0.5, zRange.min);
      if (this.yValueLabel(y)){
  ...
  if (this.showZAxis) {
    ...
    while (!step.end()) {
      ...
      if (this.zValueLabel(z)){
        let msg = this.zValueLabel(z) + ' ';
        var from3d = new Point3d(xText, yText, z+0.5);
  ...
  if (zLabel.length > 0 && this.showZAxis) {
    offset = 40;  // pixels.  // TODO: relate to the max width of the values on the z axis?
    xText  = (armVector.x > 0) ? xRange.min : xRange.max;
    yText  = (armVector.y < 0) ? yRange.min : yRange.max;
...
/**
 * Draw a box element in the view with the given properties.
 *
 * @param {CanvasRenderingContext2D} ctx
 * @param {Object} point
 * @param {number} xWidth
 ...
 * @private
 */
Graph3d.prototype._redrawBox = function(ctx, point, xWidth, yWidth, zWidth, color, borderColor) {
  var surface;
  ...
  var top = [
    {point: new Point3d(point3d.x - xWidth, point3d.y - yWidth, point3d.z + zWidth)},
    {point: new Point3d(point3d.x + xWidth, point3d.y - yWidth, point3d.z + zWidth)},
    {point: new Point3d(point3d.x + xWidth, point3d.y + yWidth, point3d.z + zWidth)},
    {point: new Point3d(point3d.x - xWidth, point3d.y + yWidth, point3d.z + zWidth)}
  ];
...
Graph3d.prototype._redrawDotColorGraphPoint = function(ctx, point) {
  ...
  var yWidth = 0.4;
  var zWidth = 0.05;
  this._redrawBox(ctx, point, xWidth, yWidth, zWidth, colors.fill, colors.border);
};
